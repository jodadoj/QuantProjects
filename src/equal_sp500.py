...
def convert_sp500_data_to_csv():
    ...
    file_path = os.path.join(
        ...
        f"output/sp500_data/{TODAYS_DATE}-full_sp500_data.csv",
    )
...
def calculate_no_of_shares_to_purchase():
    ...
    for i in range(0, len(current_values.index)):
        current_values.loc[i, "Number of Shares to Buy"] = math.floor(
            position_size / current_values["Stock Price"][i]
        )
...
def output_equal_weights_to_excel():
    ...
    file_path = os.path.join(
        os.path.dirname(os.path.realpath(__file__)),
        f"output/{today}-recommended_trades.xlsx",
    )
